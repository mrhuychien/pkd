...
function renderCoverage(cov) {
    ...
    root.innerHTML = `<p class="kd-text-sm kd-text-muted">Sắp theo độ phủ thấp nhất — ưu tiên đẩy phân phối:</p>` + paged({
        rows: cov,
        pageSize: 10,
        render: (slice) => `<div style="overflow-x:auto;"><table class="kd-table kd-mt-2">
        ...
        onDraw: (el) => el.querySelectorAll('.kd-sp-cov-drill').forEach((a) =>
            a.addEventListener('click', () => drillCoverage(a.dataset.code, a.dataset.name))),
...
function renderSkuTable() {
    ...
    const bindSort = (el) => el.querySelectorAll('th[data-sk]').forEach((th) => th.addEventListener('click', () => {
    ...
    root.innerHTML = paged({
        rows,
        pageSize: 10,
        render: (slice) => `<div style="overflow-x:auto;"><table class="kd-table">
...
async function loadSlow() {
    ...
    try {
        ...
        root.innerHTML = !list.length ? '<div class="kd-text-muted">Không có SKU chậm trong ngưỡng này.</div>' : paged({
            rows: list,
            pageSize: 10,
            render: (slice) => `
            ...
            <tbody>${slice.map((r) => `<tr><td data-label="SKU">${escapeHtml(r.item_name)}</td><td data-label="Bán cuối">${escapeHtml(r.last_sold)}</td><td data-label="Số ngày" class="kd-text-end">${r.days_since}</td><td data-label="Thùng" class="kd-text-end">${formatNumber(r.qty)}</td></tr>`).join('')}</tbody></table>`,
        });
    } catch (err) { root.innerHTML = `<div class="kd-text-muted">${escapeHtml(err.message)}</div>`; }
}
...
async function loadDepth() {
    ...
    try {
        ...
        root.innerHTML = !rows.length ? '<div class="kd-text-muted">Chưa có dữ liệu.</div>' :
            `<p class="kd-text-sm kd-text-muted">⚠️ = danh mục mỏng (< ${d.thin} SKU) → ưu tiên cross-sell.</p>` + paged({
                rows,
                pageSize: 10,
                render: (slice) => `
            ...
            <tbody>${slice.map((r) => `<tr><td data-label="${noun}">${r.thin ? '⚠️ ' : ''}${escapeHtml(r.customer_name)}</td><td data-label="Tỉnh">${escapeHtml(r.territory || '—')}</td><td data-label="Số SKU" class="kd-text-end">${r.sku_count}</td><td data-label="Doanh số" class="kd-text-end">${formatCurrency(r.revenue)}</td></tr>`).join('')}</tbody></table>`,
            });
    } catch (err) { root.innerHTML = `<div class="kd-text-muted">${escapeHtml(err.message)}</div>`; }
}
...
async function loadWhiteSpace() {
    ...
    try {
        ...
        root.innerHTML = `<div class="kd-text-sm kd-text-muted">${list.length} ${noun} có doanh số nhưng CHƯA mua "${escapeHtml(group)}" → ưu tiên chào hàng:</div>` + paged({
            rows: list,
            pageSize: 10,
            render: (slice) => html`
